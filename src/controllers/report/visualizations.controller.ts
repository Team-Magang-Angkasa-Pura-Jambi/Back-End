...
export const MeterRankController = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const EnergyOutlookController = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getUnifiedComparisonController = async (
  ...
  try {
    ...
    const result = await getUnifiedComparisonService(energyTypeName, year, month);
...
export const getTrentCounsumptionController = async (
  ...
  try {
    const { energyTypeName, year, month, meterId } = res.locals.validatedData.query;
    const result = await getTrentConsumptionService(energyTypeName, year, month, meterId);
    res200({
      res,
      data: result,

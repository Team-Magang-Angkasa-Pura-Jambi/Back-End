...
export function startEfficiencyTargetScheduler() {
  console.log('⏰ Cron job untuk pengaturan target efisiensi bulanan diaktifkan.');
  ...
  schedule(
    ...
    async () => {
      ...
      console.log(
        `[CRON - EfficiencyTarget] Memulai tugas pada ${jobStartDate.toLocaleString('id-ID', {
      ...
      try {
        ...
        const calcStartDate = new Date(Date.UTC(targetYear, targetMonth - 1, 1));
        ...
        for (const meter of activeMeters) {
          ...
          const totalConsumption = lastMonthAggregation._sum.total_consumption?.toNumber() ?? 0;
          ...
          const newTargetValue = averageDailyConsumption * (1 - EFFICIENCY_IMPROVEMENT_PERCENTAGE);
          ...
          const activePriceScheme = meter.tariff_group.price_schemes
            ...
            .sort((a, b) => b.effective_date.getTime() - a.effective_date.getTime())[0];
          ...
          if (activePriceScheme) {
            ...
            const wbpRate =
              activePriceScheme.rates.find((r) => r.reading_type.type_name === 'WBP')?.value ??
              new Prisma.Decimal(0);
            const lwbpRate =
              activePriceScheme.rates.find((r) => r.reading_type.type_name === 'LWBP')?.value ??
              new Prisma.Decimal(0);

            if (!wbpRate.isZero() && !lwbpRate.isZero()) {
              const avgPrice = wbpRate.plus(lwbpRate).div(2);
              estimatedTargetCost = new Prisma.Decimal(newTargetValue).times(avgPrice);
            } else {
              // Untuk Air atau Fuel
              const singleRate = activePriceScheme.rates[0]?.value ?? new Prisma.Decimal(0);
              estimatedTargetCost = new Prisma.Decimal(newTargetValue).times(singleRate);
            }
          }
          ...
          await prisma.efficiencyTarget.create({
            data: {
              kpi_name: `Target Efisiensi Bulanan - ${period_start.toISOString().split('T')[0]}`,

...
export function startEfficiencyTargetScheduler() {
  ...
  schedule(
    ...
    async () => {
      ...
      console.log(
        `[CRON - EfficiencyTarget] Memulai tugas pada ${jobStartDate.toISOString()}`
      );
      ...
      try {
        ...
        if (createdTargetsCount > 0) {
          const message = `Sistem telah secara otomatis membuat ${createdTargetsCount} target efisiensi baru untuk periode ${period_start.toLocaleDateString()} - ${period_end.toLocaleDateString()}.`;
          for (const admin of admins) {
            await notificationService.create({
              user_id: admin.user_id,
              title: 'Target Efisiensi Bulanan Dibuat',
              message,
              link: '/dashboard/efficiency-targets',
            });
          }
        }
        console.log(`[CRON - EfficiencyTarget] Tugas selesai. ${createdTargetsCount} target dibuat/diperbarui.`);
      } catch (error) {

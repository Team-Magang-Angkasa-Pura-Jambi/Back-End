...
> {
  ...
  public override async create(
    ...
    return this._handleCrudOperation(async () => {
      ...
      if (parent_budget_id) {
        ...
        // 2. Pastikan periode anak berada di dalam periode induk
        if (
          budgetData.period_start < parentBudget.period_start ||
          budgetData.period_end > parentBudget.period_end
        ) {
          throw new Error400(
  ...
  public async getDetailedBudgetById(budgetId: number) {
    return this._handleCrudOperation(async () => {
      ...
      if (isParentBudget) {
        ...
        for (const child of budget.child_budgets as any[]) {
          ...
          if (childMeterIds.length > 0) {
            ...
            totalRealization = totalRealization.plus(
              childRealizationResult._sum.total_cost ?? 0
            );
          }
        }
  ...
  public async getDetailedBudgets(args: Prisma.AnnualBudgetFindManyArgs) {
    return this._handleCrudOperation(async () => {
      // 1. Ambil semua data anggaran dasar
      const budgets = await this._prisma.annualBudget.findMany({
        ...
        include: {
          ...
          // BARU: Sertakan juga data anak dan induk untuk memberikan konteks
          child_budgets: {
            orderBy: { period_start: 'asc' },
            // PERBAIKAN: Sertakan alokasi dari setiap anggaran anak.
            // Ini krusial untuk menghitung total realisasi di level induk.
            include: {
              allocations: {
      ...
      // 4. Proses setiap anggaran untuk menambahkan detail kalkulasi (dibuat async)
      const detailedBudgetPromises = budgets.map(async (budget, index) => {
        const { period_start, period_end, total_budget, allocations } = budget;
        ...
        const monthlyAllocation: {
          ...
          monthName: string;
          allocatedBudget: number;
          // PERBAIKAN: Tambahkan properti realisasi untuk konsistensi
          realizationCost: number;
          remainingBudget: number;
          realizationPercentage: number | null;
        }[] = [];
        ...
        // Kalkulasi detail hanya relevan untuk anggaran periode (anak) yang punya alokasi
        // PERBAIKAN: Jalankan kalkulasi untuk semua anggaran anak, bahkan jika alokasinya kosong.
        if (!isParentBudget) {
          const periodDays =
            (period_end.getTime() - period_start.getTime()) /
              (1000 * 60 * 60 * 24) +
          ...
          const budgetPerDay =
            periodDays > 0
              ? total_budget.div(periodDays)
              : new Prisma.Decimal(0);

          const meterIds = allocations.map((alloc) => alloc.meter_id);
          const monthlyRealisations: { month: number; total_cost: number }[] =
            meterIds.length > 0 // PERBAIKAN: Hanya jalankan query jika ada meter ID
              ? await this._prisma.$queryRaw`
                  SELECT EXTRACT(MONTH FROM summary_date)::int as month, SUM(total_cost) as total_cost
                  FROM "daily_summaries"
                  WHERE meter_id = ANY(ARRAY[${Prisma.join(meterIds)}])
          ...
          for (
            ...
            if (overlapEnd >= overlapStart) {
              ...
              const remainingForMonth =
                budgetForMonth.minus(realizationForMonth);
              const percentageForMonth = budgetForMonth.isZero()
                ? null
                : realizationForMonth.div(budgetForMonth).times(100);
        ...
        // --- 2. Kalkulasi Realisasi per Meter ---
        // PERBAIKAN: Untuk parent, realisasi per meter tidak dihitung karena tidak ada alokasi langsung.
        const meterIds = allocations.map((alloc) => alloc.meter_id);
        const meterRealisations = await this._prisma.dailySummary.groupBy({
          by: ['meter_id'],
          where: {
            meter_id: { in: meterIds.length > 0 ? meterIds : [-1] }, // Gunakan -1 jika kosong
            summary_date: { gte: period_start, lte: period_end },
          },
          _sum: { total_cost: true },
        });
        ...
        const detailedAllocations = allocations.map((alloc) => {
          const allocatedBudget = total_budget.times(alloc.weight);
          const totalRealization = new Prisma.Decimal(
            realizationMap.get(alloc.meter_id) ?? 0
          );
          const remainingBudget = allocatedBudget.minus(totalRealization);
          const realizationPercentage = allocatedBudget.isZero()
            ? null
        ...
        if (isParentBudget) {
          const childMeterIds = budget.child_budgets.flatMap((cb) =>
            cb.allocations.map((a) => a.meter_id)
          );
          // PERBAIKAN: Hitung total realisasi dengan menjumlahkan realisasi dari setiap anak
          // sesuai dengan periode masing-masing anak.
          let totalRealization = new Prisma.Decimal(0);
          for (const child of budget.child_budgets) {
          ...
          if (childMeterIds.length > 0) {
            const childrenRealizationResult =
              await this._prisma.dailySummary.aggregate({
                _sum: { total_cost: true },
                where: {
                  meter_id: { in: childMeterIds },
            ...
            const totalRealization =
              childrenRealizationResult._sum.total_cost ??
              new Prisma.Decimal(0);
            const remainingBudget = total_budget.minus(totalRealization);
            const realizationPercentage = total_budget.isZero()
              ? 0
        ...
        return {
          ...
          allocations: detailedAllocations,
          monthlyAllocation,
          // Tambahkan data agregat jika ini adalah parent
          ...(isParentBudget ? { parentRealization } : {}),
        };
      });

      // Tunggu semua promise kalkulasi alokasi selesai
      const resolvedDetailedBudgets = await Promise.all(detailedBudgetPromises);

      return resolvedDetailedBudgets;
    });
  }
  /**
    ...
    return this._handleCrudOperation(async () => {
      ...
      const detailedParentBudgetsPromises = parentBudgets.map(
        async (budget) => {
          ...
          let totalRealization = new Prisma.Decimal(0);
          for (const child of budget.child_budgets) {
            const childMeterIds = (child as any).allocations.map(
              (a: any) => a.meter_id
            );
